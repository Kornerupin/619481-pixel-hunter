...
export default class Game3 extends AbstractView {
  ...
  get template() {
    return `
      <div class="game">
      <p class="game__task">Найдите ${AnswersType[this.gameData.gameQuestionsData[this.questionNumber].questionCorrectType].text} среди изображений</p>
    ...
    document.addEventListener(`mousedown`, (evt) => {
      if (document.querySelector(`.game__content--triple`)) {
        if (evt.target &&
          ...
          this.onSubmit([evt.target.getAttribute(`data-value`)]);
        }
      }
    });
  }
  ...
  onSubmit(answer) {
    game.checksAnswerForQuestion(this.gameData, [answer]);
